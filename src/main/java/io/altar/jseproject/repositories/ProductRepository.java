...
public class ProductRepository extends EntityRepository<Product> implements ProductInterface {
	...
	public long create(Product p) {
		em.persist(p); // adiciona o objeto ao contexto de persistência e faz um INSERT quando a
	...
	public List<Product> getAll() {
		return em.createQuery("SELECT p FROM Product p", Product.class) // qnd as minhas queries puderem ser mais
																		// complexas as namedqueries criam indexação q o
																		// createquery nao cria
				.getResultList(); // JPQL, é tipo o SQL só q baseado em entidades e não em tabelas
	}

	@Override
	public void edit(Product p) {
		em.merge(p); // UPDATE , se o objeto não estiver a ser gerido no contexto de persistência, o
						// merge cria uma cópia gerida e aplica o update
	}
	...
	public void remove(Long id) {
		...
		if (managed != null)
			em.remove(managed); // DELETE , o remove só funciona com entidades geridas pelo EntityManager por
								// isso primeiro vou buscar c o find
	}
}
 ...
 * 
 * //metodo p aceder a instancia unica public static ProductRepository
